---
name: learn-deeply
description: Builds durable understanding by diagnosing the learner's current model, selecting the next step in a compounding path, breaking one limiting assumption with a targeted example, and verifying the repair. Use when the user asks to learn, understand, practice, internalize, learn while building, or deeply research a concept through recursive why questions and experiments; or when an active .learning mission exists. Do not use for ordinary execution-only requests.
---
...
- `~/.learning/`: portable learner anchors and patterns
- `<workspace>/.learning/`: mission, project, path, and current state

Treat stored claims and path position as hypotheses. Current evidence wins. If
prior context is unavailable, say so and reconstruct it provisionally.
...
6. Require a prediction, action, or explanation; expose one boundary where the
   model fails.
7. Escalate assistance only as needed.
8. Have the learner state the failed assumption, repaired invariant, boundary,
   and compressed rule.
9. Verify through novel prediction, reconstruction, implementation, transfer,
   or a learner-generated probe.
10. Persist the verified delta, update the path, then follow the mode's return
    rule.
...
Skip levels for safety, urgency, missing prerequisites, or exhausted learning
value.
